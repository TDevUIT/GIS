    ...
    if (gisTerrainData?.data) {
      try {
        ...
        if (polygons.length === 0 && terrains.length > 0) {
          console.warn('âš ï¸ No terrain polygons have valid geometry.');
        } else {
          console.log(`âœ… Loaded ${polygons.length} terrain polygons`);
        }
    ...
    return (
      <div className="absolute top-20 left-1/2 -translate-x-1/2 z-[1000] bg-white/95 backdrop-blur-md px-6 py-3 rounded-lg shadow-lg border border-slate-200">
        <div className="flex items-center gap-3 text-slate-600">
          ...
          <span className="font-medium">Äang táº£i dá»¯ liá»‡u Ä‘á»‹a hÃ¬nh...</span>
        </div>
      </div>
    );
  }
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-red-600">
              {(error as any)?.message || 'KhÃ´ng thá»ƒ táº£i dá»¯ liá»‡u Ä‘á»‹a hÃ¬nh'}
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-yellow-700">
                Dá»¯ liá»‡u Ä‘á»‹a hÃ¬nh khÃ´ng cÃ³ thÃ´ng tin hÃ¬nh há»c (geometry).
              </p>
            </div>
          </div>
        </div>
      ...
      >
        <div className="space-y-2">
          <div className="flex items-center justify-between gap-4">
            <span className="text-sm font-semibold text-slate-800">Äá»™ cao (m)</span>
          ...
          <div className="pt-2 border-t text-xs text-gray-500">
            <strong>{terrainData.length}</strong> khu vá»±c
          </div>
        </div>
      </div>
            ...
            onEachFeature={(feature, layer) => {
              layer.bindPopup(`
                <div class="p-2 min-w-[200px]">
                  <h3 class="font-bold text-base mb-2">Äá»‹a hÃ¬nh</h3>
                  <div class="space-y-1.5 text-sm">
                    <div class="flex justify-between items-center">
                      <span class="text-gray-600">Äá»™ cao:</span>
                      <span class="font-semibold text-gray-800">
                        ${polygon.elevation.toFixed(1)} m
                      </span>
                    </div>
                    <div class="flex justify-between items-center">
                      <span class="text-gray-600">Má»©c Ä‘á»™:</span>
                    ...
                    <div class="flex justify-between items-center">
                      <span class="text-gray-600">Äá»™ dá»‘c:</span>
                      <span class="text-xs text-gray-600">
                        ${polygon.slope.toFixed(1)}Â° (${getSlopeLabel(polygon.slope)})
                      </span>
                    </div>
                    ${polygon.aspect !== null ? `
                      <div class="flex justify-between items-center">
                        <span class="text-gray-600">HÆ°á»›ng:</span>
                        <span class="text-xs text-gray-600">${polygon.aspect.toFixed(0)}Â°</span>
                      </div>
                    ` : ''}
                  </div>
                </div>
