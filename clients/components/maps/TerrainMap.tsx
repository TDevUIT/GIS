...
import { useGisTerrains } from '@/hooks/api/useGisTerrainsQuery';
import {
...
export default function TerrainMap({ onPolygonClick }: TerrainMapProps) {
  ...
  const { data: gisTerrainData, isLoading, error } = useGisTerrains();
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-red-600">
              {(error as any)?.message || 'Không thể tải dữ liệu địa hình'}
            </p>
          </div>
        </div>
      </div>
