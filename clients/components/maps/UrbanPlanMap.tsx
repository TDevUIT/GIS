...
import { useGisUrbanPlans } from '@/hooks/api/useGisUrbanPlansQuery';
import {
...
export default function UrbanPlanMap({ onPolygonClick }: UrbanPlanMapProps) {
  ...
  const { data: gisUrbanPlanData, isLoading, error } = useGisUrbanPlans();
    ...
    if (gisUrbanPlanData?.data) {
      try {
        ...
        if (polygons.length === 0 && urbanPlans.length > 0) {
          console.warn('⚠️ No urban plan polygons have valid geometry.');
        } else {
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-red-600">
              {(error as any)?.message || 'Không thể tải dữ liệu quy hoạch đô thị'}
            </p>
          </div>
        </div>
      </div>
