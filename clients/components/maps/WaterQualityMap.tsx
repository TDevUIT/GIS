...
import { useGisWaterQualities } from '@/hooks/api/useGisWaterQualitiesQuery';
import {
...
export default function WaterQualityMap({ onPointClick }: WaterQualityMapProps) {
  ...
  const { data: gisWaterQualityData, isLoading, error } = useGisWaterQualities();

  useEffect(() => {
    if (gisWaterQualityData?.data) {
      try {
        const waterQualities = Array.isArray(gisWaterQualityData.data)
          ? gisWaterQualityData.data
          : [gisWaterQualityData.data];
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-red-600">
              {(error as any)?.message || 'Không thể tải dữ liệu chất lượng nước'}
            </p>
          </div>
        </div>
      </div>
  ...
  const hasRawData = gisWaterQualityData?.data && (
    Array.isArray(gisWaterQualityData.data) ? gisWaterQualityData.data.length > 0 : true
  );
