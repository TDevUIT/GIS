...
import { useGisInfrastructures } from '@/hooks/api/useGisInfrastructuresQuery';
import {
...
export default function InfrastructureMap({ onPointClick }: InfrastructureMapProps) {
  ...
  const { data: gisInfrastructureData, isLoading, error } = useGisInfrastructures();

  useEffect(() => {
    if (gisInfrastructureData?.data) {
      try {
        const infrastructures = Array.isArray(gisInfrastructureData.data)
          ? gisInfrastructureData.data
          : [gisInfrastructureData.data];
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-red-600">
              {(error as any)?.message || 'Không thể tải dữ liệu cơ sở hạ tầng'}
            </p>
          </div>
        </div>
      </div>
  ...
  const hasRawData = gisInfrastructureData?.data && (
    Array.isArray(gisInfrastructureData.data) ? gisInfrastructureData.data.length > 0 : true
  );
