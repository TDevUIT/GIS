...
export class UrbanPlansService {
  ...
  async create(createDto: any) {
    const url = `${this.gisServerUrl}/urban-plans`;
  ...
  async findOne(id: string) {
    const url = `${this.gisServerUrl}/urban-plans/${id}`;
    const response = await firstValueFrom(
  ...
  async findAtPoint(lng: string, lat: string) {
    const url = `${this.gisServerUrl}/urban-plans/at-point`;
    const response = await firstValueFrom(
      this.httpService
  ...
  async findIntersecting(wktBody: any) {
    const url = `${this.gisServerUrl}/urban-plans/intersects-with`;
    const response = await firstValueFrom(
