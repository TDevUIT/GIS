...
export class PopulationsProcessor extends WorkerHost {
  private readonly logger = new Logger(PopulationsProcessor.name);
  ...
  constructor(
    ...
    if (!url) throw new Error('GIS_SERVER_URL is not defined');
    this.gisServerUrl = url;
  }

  async process(job: Job<unknown, any, string>): Promise<any> {
    switch (job.name) {
      case 'simulate-yearly-population-growth':
        return this.handleSimulateYearlyPopulationGrowth(job);
      default:
        throw new Error(`Unknown job name: ${job.name}`);
    }
  }

  private async handleSimulateYearlyPopulationGrowth(
    ...
    try {
      ...
      for (const district of districts) {
        ...
        if (populationHistory.length === 0) {
          this.logger.log(
            `District "${district.name}" has no population data. Bootstrapping...`,
          );
          ...
          this.logger.log(
            `Bootstrapped population for "${district.name}" in ${currentYear}: ${newPopulationDto.populationTotal}`,
          );
          simulatedCount++;
          continue;
        }
        ...
        if (
          ...
          this.logger.warn(
            `Latest record for "${district.name}" (year ${latestRecord.year}) is missing detailed data. Skipping.`,
          );
          continue;
        }
        ...
        await this.postToGis('/populations', newPopulationDto);
        this.logger.log(
          `Simulated population data for "${district.name}" in ${currentYear}: ${newPopulationTotal} people.`,
        );
        simulatedCount++;
      }

      this.logger.log(
        `--- [${job.name}] Finished Job. Processed data for ${simulatedCount} districts. ---`,
      );
    } catch (error: any) {
      this.logger.error(
        `[${job.name}] Failed to run job`,
  ...
  private simulateHouseholds(
    ...
    const sampleSize = Math.min(newTotal, 500);
    for (let i = 0; i < sampleSize; i++) {
      newHouseholds.push({
        householdSize: sizePool[Math.floor(Math.random() * sizePool.length)],
        incomeLevel: incomePool[Math.floor(Math.random() * incomePool.length)],
