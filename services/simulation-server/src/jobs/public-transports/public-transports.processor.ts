...
export class PublicTransportsProcessor extends WorkerHost {
  private readonly logger = new Logger(PublicTransportsProcessor.name);
  ...
  async process(job: Job<unknown, any, string>): Promise<any> {
    switch (job.name) {
      case 'simulate-network-expansion':
        return this.handleSimulateNetworkExpansion(job);
      case 'optimize-route-operations':
        return this.handleOptimizeRouteOperations(job);
      default:
        throw new Error(`Unknown job name: ${job.name}`);
    }
  }

  private async handleSimulateNetworkExpansion(job: Job<unknown, any, string>) {
  ...
  private async handleOptimizeRouteOperations(job: Job<unknown, any, string>) {
    this.logger.log(`--- [${job.name}] Starting Job ---`);
    try {
      const allRoutes =
        await this.fetchFromGis<PublicTransport[]>('/public-transports');
      if (!allRoutes || allRoutes.length === 0) {
        this.logger.warn('No routes to optimize. Skipping.');
        return;
      }
      ...
      for (const route of routesToOptimize) {
        ...
        if (Object.keys(updatePayload).length > 0) {
          this.logger.log(
            `Optimizing route "${route.routeName}": ${JSON.stringify(updatePayload)}`,
          );
          await this.patchToGis(
            `/public-transports/${route.id}`,
            updatePayload,
          );
        }
      }
      this.logger.log(
        `--- [${job.name}] Finished Job. Processed ${routesToOptimize.length} routes. ---`,
      );
    } catch (error) {
      this.logger.error(`[${job.name}] Failed to run job`, error.stack);
      throw error;
