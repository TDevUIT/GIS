<template>
    <div class="absolute top-4 right-4 z-[1000] p-3 bg-gray-800/80 backdrop-blur-sm rounded-lg shadow-2xl border border-gray-700/50">
        <h3 class="text-white font-bold text-sm mb-2 border-b border-gray-600 pb-1">Land Use Legend</h3>
        <ul class="space-y-1">
...
import { getLandUseTypeStyle } from '~/utils/landUseStyles';
...
const uniqueTypes = computed(() => {
    const unique = [...new Set(props.types)]
    const styles: Record<string, LegendItem> = {}
    unique.forEach(type => {
        const style = getLandUseTypeStyle(type).value
        if (style) {
            styles[type] = style
        }
    })
