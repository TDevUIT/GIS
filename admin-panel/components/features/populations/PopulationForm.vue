    ...
    <form @submit.prevent="onFormSubmit" class="space-y-8">
        ...
        <div class="p-6 rounded-lg border border-gray-700 bg-gray-800/50">
            ...
            <div v-for="(demo, index) in formState.demographics" :key="`demo-${index}`" class="grid grid-cols-12 gap-4 items-center mb-3">
                <input v-model="demo.ageGroup" placeholder="Age Group (e.g., 0-14)" required class="col-span-12 sm:col-span-5 form-input" />
        ...
        <div class="p-6 rounded-lg border border-gray-700 bg-gray-800/50">
            ...
            <div v-for="(household, index) in formState.households" :key="`hh-${index}`" class="grid grid-cols-12 gap-4 items-center mb-3">
                ...
                <select v-model="household.incomeLevel" class="col-span-6 sm:col-span-4 form-select">
                    ...
                    <option v-for="level in incomeLevels" :key="level" :value="level">{{ level }}</option>
                </select>
                <select v-model="household.housingType" class="col-span-6 sm:col-span-4 form-select">
                    ...
                    <option v-for="type in housingTypes" :key="type" :value="type">{{ type }}</option>
                </select>
...
const incomeLevels = ['Thấp', 'Trung bình', 'Cao'];
const housingTypes = ['Nhà riêng', 'Chung cư cao cấp', 'Nhà trong hẻm', 'Nhà trọ'];
...
const formState = reactive<CreatePopulationDTO>({
    ...
    demographics: props.initialData?.demographics?.map(d => ({ ageGroup: d.ageGroup, male: d.male, female: d.female })) || [],
    households: props.initialData?.households?.map(h => ({ householdSize: h.householdSize || 1, incomeLevel: h.incomeLevel, housingType: h.housingType })) || [],
});

function addDemographic() {
    formState.demographics?.push({ ageGroup: '', male: 0, female: 0 });
}
...
function onFormSubmit() {
    const cleanedHouseholds = formState.households?.filter(h => h.householdSize > 0 && h.incomeLevel && h.housingType);
    emit('submit', { ...formState, households: cleanedHouseholds });
}
</script>
