...
// Các import không thay đổi, giữ nguyên
...
useAsyncData('districts-for-filter', async () => {
    ...
    const districtData = response.data.data; // API districts có 2 tầng data
...
const { refresh: refreshPopulations } = useAsyncData(
    ...
    async () => {
        ...
        if (selectedDistrictId.value) {
            params.districtId = selectedDistrictId.value;
        }
        if (selectedYear.value) {
            params.year = parseInt(selectedYear.value, 10);
        }

        const response = await $api.populations.getAll(params);
        allPopulations.value = response.data.data || [];

        // Logic tự động chọn item
        if (allPopulations.value.length === 1 && !selectedPopulation.value) {
            // Khi chỉ có 1 kết quả, fetch chi tiết và hiển thị
            const detailedResponse = await $api.populations.getById(allPopulations.value[0].id);
            selectedPopulation.value = detailedResponse.data.data;
        } else if (selectedPopulation.value && !allPopulations.value.some(p => p.id === selectedPopulation.value?.id)) {
            // Nếu item đang chọn không còn trong list, bỏ chọn
...
watch(selectedPopulation, async (newSelection) => {
    if (newSelection && newSelection.id) { // Thêm kiểm tra newSelection.id
        try {
            // Kiểm tra xem dữ liệu chi tiết đã có chưa
            if (!newSelection.demographics || !newSelection.households) {
                // Nếu chưa có, gọi API getById để lấy đầy đủ
                const response = await $api.populations.getById(newSelection.id);
                selectedPopulation.value = response.data.data; // Cập nhật lại với dữ liệu đầy đủ
            }
...
async function handleRowClick(population: Population) {
    if (selectedPopulation.value?.id !== population.id) {
        // Chỉ gán object cơ bản, watcher sẽ lo việc fetch chi tiết
